...
import { stages } from "@/components/theme";
...
const computedMessage = computed(() =>
  props.stageMessage ?? stage.value?.message ?? ""
);

/* FINAL classes — fully stable */
const inputClasses = computed(() => {
  const sizeClass = sizes[props.size ?? "md"];
  const stageClass = stage.value?.border ?? "";

  return [baseClass, sizeClass, stageClass]
